...
export class NodePluginClass implements Plugin {
    ...
    constructor(public options: NodePluginOptions) {
        ...
        const identifier = options.identifier || path.basename(options.file);
        this.dest = `node/${identifier}`;
        this.resolve = `/${this.dest}/`;
    ...
    protected copyDependencies(context: WorkFlowContext, patterns: string[], sourcePathAbs: string, targetDirAbs: string): void {
        patterns.forEach((pattern) => {
            glob.sync(path.join(sourcePathAbs, pattern), { absolute: true })
    ...
    public transform(file: File) {
        ...
        file.alternativeContent = (`module.exports = require(".${joinFuseBoxPath(this.resolve, userFile)}");`);
        ...
        return new Promise((resolve: any, reject: any) => {
            readFile(file.absPath, (err, data) => {
                ...
                return context.output.writeToOutputFolder(userPath, data, true).then(result => {
                    if (result.hash) {
                        file.alternativeContent = (`module.exports = require(".${joinFuseBoxPath(this.resolve, result.filename)}");`);
                    }
